import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import React from 'react'


export default function Book({ book }) {
const stars =   new Array(Math.floor(book.rating)).fill(0);
// console.log(stars)
    return (
        <div className="book">
            <a href="/">
                <figure className="book__img--wrapper">
                    <img src={book.url} alt="" className="book__img" /></figure>
            </a>
            <div className="book__title">
                <a href="/" className="book__title--link">{book.title}</a>
            </div>
            <div className="book__ratings icon">
                {
                  stars.map((_, index) =>   <FontAwesomeIcon key={index} icon="star" className='book__rating' />
                
                )}
                {
                    !Number.isInteger(book.rating) && <FontAwesomeIcon icon="star-half-alt" className='book__rating' />
                }
               
                {/* <FontAwesomeIcon icon="star" className='book__rating' />
                <FontAwesomeIcon icon="star-half-alt" className='book__rating' /> */}
            </div>
            <div className="book__price">
                {book.salePrice ? (
                    <>
                        <span className="book__price--normal">${book.originalPrice.toFixed(2)}</span>${book.salePrice.toFixed(2)}
                    </>)
                    
                    : ( <>${book.originalPrice.toFixed(2)}</> )

                }



            </div>
        </div>
    )
}
